...
var Db = module.exports = function(dbname, server) {
  ...
  var iface = {
    ...
    close: function(force, callback) {
      ...
      iface.emit('close');
      iface.removeAllListeners('close');
    ...
    createIndex: function (name, keys, options, callback) {
      ...
      iface.createCollection(name, {}, function (err, collection) {
    ...
    dropCollection: function(name, callback) {
      iface.collection(Db.SYSTEM_NAMESPACE_COLLECTION).deleteOne({name:name}, function (err, result) {
        if (!result.deletedCount) return callback(new Error('ns not found'));

        var ns = dbname+'.'+name;
        iface.collection(Db.SYSTEM_INDEX_COLLECTION).deleteMany({ns:ns}, function (err, result) {
    ...
    listCollections: function(filter, options) {
      ...
      return iface.collection(Db.SYSTEM_NAMESPACE_COLLECTION).find(filter);
    },
  ...
  iface.__proto__ = EventEmitter.prototype;
    ...
    function clearCollections() {
        backingstore.collections.clear();
    }
  ...
  function getCollection(name) {
    ...
    instance = Collection(iface, state);
  ...
  var open = false;
  var backingstore = server.databases[dbname] || (server.databases[dbname] = create_backingstore(iface));
